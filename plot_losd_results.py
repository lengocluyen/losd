...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create visualizations from LOSD result CSV files."
    )
    parser.add_argument(
        "--results-dir",
        default="exp_out/losd",
        help="Directory containing losd_candidate_records.csv and losd_parent_results.csv.",
    )
    parser.add_argument(
        "--output-dir",
        default="",
...
def load_result_frames(results_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    candidate_path = results_dir / "losd_candidate_records.csv"
    parent_path = results_dir / "losd_parent_results.csv"
    parent_summary_path = results_dir / "losd_method_summary.csv"
    candidate_summary_path = results_dir / "losd_candidate_summary.csv"
...
def plot_method_performance(parent_summary_df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    ax.set_title("LOSD Performance by Prompt Mode and Variant")
    ax.set_ylabel("Score")
