...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the journal extension neuro-symbolic refinement pipeline on cached notebook outputs."
    )
...
def build_config(args: argparse.Namespace) -> JournalExtensionConfig:
    return JournalExtensionConfig(
...
def main() -> None:
    ...
    target_outdir = Path(args.target_outdir) if args.target_outdir else source_outdir / "journal_extension"
    ...
    pipeline = JournalExtensionPipeline(resources, embedder, config)
    ...
    candidate_path = target_outdir / "journal_candidate_records.csv"
    parent_path = target_outdir / "journal_parent_results.csv"
    parent_summary_path = target_outdir / "journal_method_summary.csv"
    candidate_summary_path = target_outdir / "journal_candidate_summary.csv"
